...
app.post("/push", function(req, res) {
  ...
  let new_values = req.body.values.toString().split(",");

  for (let i = 0; i < new_values.length; ++i) {
    stack.push(parseInt(new_values[i]));
  }
...
app.get("/add", function(req, res) {
  ...
  let answer = stack[stack.length - 1] + stack[stack.length - 2]
  stack.pop([stack.length - 1]);
  stack.pop([stack.length - 2]);
  stack.push(answer);
...
app.get("/subtract", function(req, res) {
  ...
  let answer = stack[stack.length - 2] - stack[stack.length - 1]
  stack.pop([stack.length - 1]);
  stack.pop([stack.length - 2]);
  stack.push(answer);
  res.send(JSON.stringify(""));
